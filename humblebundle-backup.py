...
import requests, six.moves.http_cookiejar, os, sys, json, six.moves.urllib.request, six.moves.urllib.error, six.moves.urllib.parse
...
if not list_missing_only:
    ...
    while proceed.lower() != "y":
        if proceed.lower() == "n":
            sys.exit()
        proceed = input("Thats not a valid option. Continue? [y/n]: ")
